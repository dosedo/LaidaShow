...
#ifndef WCLRECORDENGINE
#define WCLRECORDENGINE
...
- (BOOL)isSessionRunning{
    return self.recordSession.isRunning;
}
...
- (void)doPinch {
    ...
    self.inputDevice.device.activeFormat.videoMaxZoomFactor;
    if( _scaleNum > maxZoomFactor ){
        _scaleNum = maxZoomFactor;
    }
    
    AVCaptureDevice *device = self.inputDevice.device;
    NSError *error;
    if ([device lockForConfiguration:&error]) {
        [device rampToVideoZoomFactor:_scaleNum withRate:4.0];
