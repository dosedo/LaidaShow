...
+ (NSString *)mj_localizedStringForKey:(NSString *)key value:(NSString *)value
{
    ...
    BOOL needRefreshBundle = ![self mj_needReloadBundle];
    //    if (bundle == nil) {
        
    if( needRefreshBundle ){
        // （iOS获取的语言字符串比较不稳定）目前框架只处理en、zh-Hans、zh-Hant三种情况，其他按照系统默认处理
//        NSString *language = [NSLocale preferredLanguages].firstObject;
#if 1
        //获取切换所选择的语言
//        NSString *language = [[NSUserDefaults standardUserDefaults] objectForKey:@"jhAppLanguage"];
        NSString *language = [TSLanguageModel currLanguageModel].languageCode;
#else
        //（iOS获取的语言字符串比较不稳定）目前框架只处理en、zh-Hans、zh-Hant三种情况，其他按照系统默认处理
        NSString *language = [NSLocale preferredLanguages].firstObject;
#endif
