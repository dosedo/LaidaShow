...
@interface DSAppleLogin () <ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding>

@end

@implementation DSAppleLogin

+ (instancetype)shareAppleLogin{
    static DSAppleLogin *lo = nil;
...
- (UIView *)getLoginBtnWithFrame:(CGRect)fr target:(nonnull id)target sel:(nonnull SEL)sel{
...
//        appleIDBtn.frame = fr;
//        appleIDBtn.layer.cornerRadius = 20.f;
//        appleIDBtn.layer.masksToBounds = YES;
//
//        if( target && sel && [ target respondsToSelector:sel] ){
//            [appleIDBtn addTarget:target action:sel forControlEvents:UIControlEventTouchUpInside];
//        }
//        return appleIDBtn;
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization API_AVAILABLE(ios(13.0)){
    ...
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        if( self.competeBlock ){
            self.competeBlock(user, familyName?familyName:givenName);
        }
