...
#warning wkunadd
NSData* DecodeAMRToWAVEPath(NSData* data, NSString *filePath) {
    NSMutableData* fpwave = nil;
    void * destate;
    int nFrameCount = 0;
    int stdFrameSize;
    ...
    NSString * DATA = [[NSString alloc] initWithData:data encoding:CFStringConvertEncodingToNSStringEncoding (kCFStringEncodingGB_18030_2000)];
    //有可能是android 3gp格式
    if (!strncmp(rfile, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER)))
    {
        data = fuckAndroid3GP(data);
